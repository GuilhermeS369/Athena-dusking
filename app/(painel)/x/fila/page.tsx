...
export default async function TwitterQueuePage() {
  ...
  return <main className="standalone-page queue-page"><header className="queue-page-header"><div><span>{context.activeOrganization.name} · X / Twitter</span><h1>Fila de publicação</h1></div><a className="button button-ghost" href="/x/postagem">Nova postagem</a></header><TwitterQueueClient organizationId={organizationId} programs={programs.data ?? []} profiles={profiles.data ?? []} groups={groups.data ?? []} memberships={memberships.data ?? []} shortfalls={shortfalls.data ?? []} items={items.data ?? []} summary={summary.error ? null : summary.data ?? null} canEdit={context.activeOrganization.role !== 'viewer'} /></main>;
}
