...
export default async function TwitterGroupsPage() {
  ...
  // Fixado fora do callback: dentro dele o TypeScript perde o estreitamento de
  // activeOrganization feito no guard acima.
  ...
  // Uma página de 100 grupos pode cobrir milhares de vínculos (relação 1:N), bem
  // acima do teto de 1.000 linhas do PostgREST.
  const members=groupIds.length||profileIds.length?await fetchAllRows((from,to)=>admin.from('twitter_group_members').select('group_id,profile_id,created_at').eq('organization_id', organizationId).or(`${groupIds.length?`group_id.in.(${groupIds.join(',')})`:''}${groupIds.length&&profileIds.length?',':''}${profileIds.length?`profile_id.in.(${profileIds.join(',')})`:''}`).order('group_id').order('profile_id').range(from,to)):{data:[],error:null as null};
