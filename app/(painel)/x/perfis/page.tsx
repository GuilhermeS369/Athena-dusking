...
export default async function TwitterProfilesPage() {
  ...
  const [profilesResult, groupsResult, membersResult, queueResult, connectionsResult, intentsResult, legacyAttemptsResult, mirrorResult] = await Promise.all([
    ...
    admin.from('twitter_connections').select('id,identity_id,label,status,twitter_slot_limit,remote_twitter_account_count,remote_inventory_checked_at,last_error_code,last_error_message,last_sync_at').eq('organization_id', organizationId).is('deleted_at', null).order('created_at', { ascending: false }),
  ...
  const [{ data: wallets }, { data: connectionProfiles }] = await Promise.all([
    identityIds.length ? admin.from('twitter_wallets').select('identity_id,posted_balance_micros,reserved_micros').eq('organization_id', organizationId).in('identity_id', identityIds) : Promise.resolve({ data: [] }),
