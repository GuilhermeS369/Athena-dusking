...
export default async function TwitterZernioPage() {
  ...
  const[connectionsResult,identitiesResult,eventsResult]=await Promise.all([
    admin.from('twitter_connections').select('id,identity_id,label,status,last_sync_at,last_error_message').eq('organization_id',organizationId).is('deleted_at',null).order('created_at',{ascending:false}),
