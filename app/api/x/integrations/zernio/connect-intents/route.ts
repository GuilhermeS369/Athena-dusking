...
export async function POST(request: Request) {
  ...
  if (body.mode === 'bulk') {
    ...
    const [{ data: connections }, { data: groups }, { data: profiles }, { data: intents }, { data: legacyAttempts }] = await Promise.all([
      admin.from('twitter_connections').select('id,label,twitter_slot_limit,remote_twitter_account_count,remote_inventory_checked_at,last_error_code').eq('organization_id', organizationId).is('deleted_at', null),
      admin.from('twitter_groups').select('id,name').eq('organization_id', organizationId).is('deleted_at', null),
      admin.from('twitter_profiles').select('current_connection_id').eq('organization_id', organizationId).is('deleted_at', null),
