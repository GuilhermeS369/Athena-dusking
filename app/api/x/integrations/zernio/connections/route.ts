...
export async function GET() {
  ...
  const [{ data: wallets }, { data: grants }, { data: profiles }, { data: attempts }] = await Promise.all([
    ...
    connectionIds.length ? admin.from('twitter_connection_oauth_attempts').select('connection_id').eq('organization_id', organizationId).in('connection_id', connectionIds).eq('status', 'pending').gt('expires_at', new Date().toISOString()) : Promise.resolve({ data: [] }),
  ]);
