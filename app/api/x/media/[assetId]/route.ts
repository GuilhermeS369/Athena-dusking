...
export async function DELETE(
  ...
  if (error)
    return NextResponse.json(
      { error: "Não foi possível remover o asset X." },
      { status: 500 },
    );
  await admin
  ...
  if (referenceError) {
    return NextResponse.json({
      ...
      warning: "Mídia ocultada; objeto preservado por segurança.",
    });
  }
  if ((count ?? 0) > 0)
    return NextResponse.json({
      ...
      storageRetained: true,
      warning:
        "Mídia removida da galeria; o arquivo foi preservado porque pertence a um programa X confirmado.",
    });
  ...
  return NextResponse.json({
    ...
    warning: storageError
      ? "Mídia ocultada; a limpeza do objeto será reconciliada depois."
      : undefined,
  });
}
