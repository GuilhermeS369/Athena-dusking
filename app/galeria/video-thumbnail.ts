...
function videoReadError(video: HTMLVideoElement) {
  ...
  const reason = code === MediaError.MEDIA_ERR_ABORTED ? 'leitura cancelada'
    : code === MediaError.MEDIA_ERR_NETWORK ? 'falha de leitura do arquivo'
      : code === MediaError.MEDIA_ERR_DECODE ? 'codec ou arquivo não pôde ser decodificado'
        : code === MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED ? 'formato ou codec não suportado pelo navegador'
          : 'erro desconhecido do navegador';
  return new Error(`Não foi possível ler os quadros deste vídeo (${reason}${video.error?.message ? `: ${video.error.message}` : ''}).`);
}
...
function waitFor(
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    const timeout = window.setTimeout(() => {
      ...
      else reject(new Error(`Tempo esgotado ao preparar a miniatura do vídeo (${event}; readyState=${video.readyState}, networkState=${video.networkState}).`));
    }, timeoutMs);
    const cleanup = () => {
    ...
    const onError = () => { cleanup(); reject(videoReadError(video)); };
...
function canvasToBlob(canvas: HTMLCanvasElement) {
  return new Promise<Blob>((resolve, reject) => {
    canvas.toBlob((blob) => blob ? resolve(blob) : reject(new Error('O navegador não conseguiu criar a imagem da miniatura.')), 'image/jpeg', JPEG_QUALITY);
  });
}
...
export async function createVideoFallbackThumbnail(fileName: string) {
  ...
  return new File([blob], `${fileName.replace(/\.[^.]+$/, '')}-thumb.jpg`, { type: 'image/jpeg' });
}

function evaluateFrame(context: CanvasRenderingContext2D, width: number, height: number) {
  ...
  return { brightness, contrast: Math.sqrt(Math.max(0, totalSquared / count - brightness * brightness)) };
}

async function captureFrame(video: HTMLVideoElement, time: number, fileName: string): Promise<Frame> {
  ...
  const seeked = waitFor(video, 'seeked', VIDEO_SEEK_TIMEOUT_MS, () => (
    !video.seeking
  ...
  if (!sourceWidth || !sourceHeight) throw new Error('O vídeo não possui dimensões utilizáveis.');
  ...
  const blob = await canvasToBlob(canvas);
  return { file: new File([blob], `${fileName.replace(/\.[^.]+$/, '')}-thumb.jpg`, { type: 'image/jpeg' }), ...metrics };
}
...
export async function createRealVideoThumbnail(file: File) {
  ...
  try {
    ...
    const metadata = waitFor(video, 'loadedmetadata', VIDEO_METADATA_TIMEOUT_MS, () => video.readyState >= HTMLMediaElement.HAVE_METADATA);
    video.src = url;
    video.load();
    await metadata;
    if (!Number.isFinite(video.duration) || video.duration <= 0) throw new Error('A duração do vídeo não pôde ser determinada.');
    ...
    for (const candidate of candidates) {
      try { frames.push(await captureFrame(video, candidate, file.name)); } catch { /* tenta o próximo ponto */ }
    }
    if (!frames.length) throw new Error('Nenhum quadro do vídeo pôde ser capturado.');
    const usable = frames.filter((frame) => frame.brightness > 12 && frame.contrast > 5);
    return (usable.length ? usable : frames).sort((left, right) => (right.contrast + right.brightness * 0.1) - (left.contrast + left.brightness * 0.1))[0].file;
  } finally {
...
export async function createVideoThumbnail(file: File) {
  try {
    ...
    console.warn('[gallery] Não foi possível capturar quadro do vídeo; usando miniatura fallback.', error);
