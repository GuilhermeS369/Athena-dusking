...
## X-0107 — capacidade confirmada, ciclo OAuth e refinamento visual `/x/zernio`

- UTC: 2026-08-24T12:49:00Z; São Paulo: 2026-08-24T09:49:00-03:00. Executor: Codex GPT-5. Entrada: branch `codex/x-twitter-module`, commit documental `8a000bf`, worktree limpo; checkpoint executável `ae8263e`.
- Objetivo: corrigir proporção/spacing das métricas e defaults, aplicar máscara monetária durante a digitação, explicar a exclusão, eliminar `—/2` quando há perfil local confirmado e tornar o TTL OAuth compreensível. Fora do escopo: saldo, grants, ledger, tabelas, migrations, workers, PM2, Zernio remoto e Instagram.
- Diagnóstico read-only: conexão `RobbieFatigate5434` com um perfil local ativo, inventário remoto nunca coletado (`null`) e attempt OAuth pendente com TTL de 15 minutos. O saldo permaneceu 16.200.000 micros, reservado 0, versão 27.
- Alterações: reserva OAuth agora inclui ID/expiração na API; contagem filtra por relógio do cliente; aviso informa horário e expiração automática; admin pode liberar attempts pendentes ainda válidos; ocupação usa `max(remoto, local) + reservas válidas`; botão virou `Sincronizar inventário`; exclusão virou `Excluir API e perfis` com efeitos completos descritos; input monetário usa máscara cent-based.
- Segurança: a rota de liberação exige admin, organização ativa, conexão não excluída e atualiza somente attempts `pending` não expirados. Nenhuma chamada ao provedor é feita. A exclusão não foi executada; o modal permaneceu bloqueado sem motivo e confirmação `EXCLUIR`.
- Validação local: 251/251 testes, TypeScript, build de 47 páginas e `git diff --check`. Warnings metadata preexistentes em login/onboarding/not-found permaneceram fora do escopo. Browser local desktop/mobile aprovou layout e ausência de overflow.
