...
Decisão de continuidade: a materialização desses itens financiados permanece dentro da confirmação RPC. A ADR-X-017 proíbe separar reserva e criação da fila em um worker de geração, preservando a atomicidade validada nesta fase.

Auditoria de contrato em 23/08/2026 UTC: corrigido um mismatch entre API e cliente. `/api/x/bulk/review` devolvia `wallets` e não devolvia `costBreakdown`, mas a tela renderizava `walletSnapshots` e `costBreakdown`; uma revisão 200 poderia quebrar a UI. O contrato agora entrega ambos os campos canônicos, schedule e shortfalls, e o cliente rejeita resposta incompleta informando que nada foi reservado. Gate local conjunto: 201/201 testes, TypeScript, build e diff check; nenhuma revisão real, reserva ou chamada externa.
