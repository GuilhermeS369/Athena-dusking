...
export type TwitterBulkRequest = { scheduleVersion:2; name:string; profileIds:string[]; texts:string[]; mediaSets:ReviewMediaSet[]; schedule:ReviewSchedule };
...
export async function prepareTwitterBulkReview(organizationId:string,input:TwitterBulkRequest,reviewedAt?:string){
  ...
  const normalized={scheduleVersion:2,name:input.name.trim(),profileIds,texts:input.texts,mediaSets:input.mediaSets,schedule:input.schedule}; const requestDigest=twitterReviewDigest(normalized);
