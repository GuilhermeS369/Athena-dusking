-- Mantém o primeiro momento em que uma mídia foi publicada com sucesso.
-- A mídia continua disponível no histórico da galeria, mas deixa de ser
-- oferecida na composição de novas publicações.

alter table public.media_assets
  add column first_published_at timestamptz;

create index media_assets_org_posted_idx
  on public.media_assets (organization_id, first_published_at desc)
