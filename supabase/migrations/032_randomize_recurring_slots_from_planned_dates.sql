-- O compositor calcula a data-base de cada recorrência para a prévia. Ela não
-- pode ser persistida literalmente: a reserva definitiva escolhe um dos nove
-- minutos da janela para o perfil, sem distinguir formato.
...
begin
  ...
  loop
    ...
    -- Logo, perfis diferentes podem receber o mesmo minuto sem conflito.
    ...
    if resolved_execute_at is null and nullif(item_json ->> 'scheduleTime', '') is not null then
      ...
      -- até a primeira janela ainda futura sem mudar a regra de nove vagas.
      ...
      loop
        ...
        from (
          -- O horário-base é um marcador de faixa; as vagas são xx:01 a xx:09.
        ...
        -- Nove publicações do mesmo perfil esgotam a janela. A décima ocupa a
        -- próxima faixa de dez minutos, preservando as anteriores.
