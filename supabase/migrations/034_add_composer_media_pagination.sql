-- Paginação da biblioteca do compositor. O filtro de uso é calculado no banco
-- para que uma página cheia tenha sempre mídias elegíveis, sem varrer a galeria
...
create index if not exists publication_item_media_asset_item_idx
  on public.publication_item_media (organization_id, media_asset_id, publication_item_id);

revoke all on function public.list_composer_media_ids(uuid, text, uuid, boolean, timestamptz, uuid, integer) from public, anon;
