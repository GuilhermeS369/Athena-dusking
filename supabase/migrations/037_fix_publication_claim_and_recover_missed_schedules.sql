-- Correção emergencial do worker.
-- A migration 029 reintroduziu uma referência ambígua a `batch_id` no CTE que
-- atualiza o lote. Como `batch_id` também é uma coluna retornada pela função,
-- Postgres aborta o claim com 42702 antes de qualquer item ser processado.
--
-- Além de qualificar `claimed.batch_id`, o claim recupera agendamentos vencidos
-- antes de selecionar candidatos. Isso impede que a versão já implantada do
-- dispatcher publique itens antigos imediatamente após a correção do RPC.
...
begin
  ...
  -- Tem de acontecer no próprio RPC para proteger também workers já em
  -- produção, que ainda não chamam a rotina de recuperação explicitamente.
  perform 1
  from public.recover_missed_publication_slots(500, 120);
  ...
  with candidates as (
    ...
    where batch_row.id in (select claimed.batch_id from claimed)
