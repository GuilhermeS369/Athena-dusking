-- Corrige o resultado de quarentena da migration 098 para que o RPC informe
-- corretamente o estado já bloqueado ou o bloqueio recém-aplicado.
...
begin
  ...
  if normalized_fingerprint = 'without_fingerprint' and p_phase = 'provider_download' then
    select coalesce(nullif(attempt.url_fingerprint, ''), normalized_fingerprint) into normalized_fingerprint
    from public.media_asset_delivery_attempts attempt
    where attempt.media_asset_id = asset_row.id and attempt.publication_item_id = item_row.id
      and attempt.phase = 'url_probe' and attempt.outcome = 'succeeded'
    order by attempt.created_at desc limit 1;
  end if;

  insert into public.media_asset_delivery_attempts (organization_id, media_asset_id, publication_item_id, provider, phase, outcome, error_code, error_message, url_fingerprint)
  values (asset_row.organization_id, asset_row.id, item_row.id, left(trim(p_provider), 80), p_phase, p_outcome,
  ...
  values (asset_row.id, asset_row.organization_id) on conflict (media_asset_id) do nothing;
  ...
  if p_outcome = 'succeeded' and p_phase = 'provider_download' then
    update public.media_asset_delivery_health health set consecutive_equivalent_failures = 0, last_success_at = now_at, updated_at = now_at
    where health.media_asset_id = asset_row.id;
  elsif p_outcome = 'succeeded' then
    update public.media_asset_delivery_health health set last_success_at = now_at, updated_at = now_at
    where health.media_asset_id = asset_row.id;
  ...
  if quarantine_now then
    ...
    loop
      perform public.log_publication_item_event(affected_item.id, 'cancelled', affected_item.previous_status, 'removed', null,
  ...
  return jsonb_build_object('mediaAssetId', asset_row.id,
    'quarantined', quarantine_now or health_row.quarantined_at is not null,
    'consecutiveEquivalentFailures', case when p_outcome = 'succeeded' and p_phase = 'provider_download' then 0 else health_row.consecutive_equivalent_failures end);
end;
$$;
