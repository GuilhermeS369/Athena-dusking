-- Impede o claim destrutivo de duplicidade sem preflight recente. Mantém a
-- migration 130 imutável depois de aplicada e recompila somente esta RPC.
...
begin
  ...
  update public.zernio_profile_recycling_jobs job set
    ...
    deferred_reason = case
      ...
      when incident.removal_preflight_at is null
        or incident.removal_preflight_at < timezone('utc', now()) - interval '10 minutes'
        then 'removal_preflight_required'
  ...
  where job.incident_id = incident.id
    ...
    and (
      ...
      or incident.removal_preflight_at is null
      or incident.removal_preflight_at < timezone('utc', now()) - interval '10 minutes'
      or exists (
    ...
    where job.status in ('pending', 'deferred', 'remote_removal_pending', 'retry_pending', 'processing')
      ...
      and (
        ...
        or (
          ...
          and incident.removal_preflight_at >= timezone('utc', now()) - interval '10 minutes'
          and incident.removal_preflight_snapshot_at >= timezone('utc', now()) - interval '10 minutes'
          and not exists (
            select 1 from public.publication_items item
            where item.organization_id = incident.organization_id
              and item.profile_id = incident.retained_profile_id
