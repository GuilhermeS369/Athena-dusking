-- A versão anterior de release devolvia o profile canônico ao pool mesmo após
-- callback com conta autorizada. Para recuperar esses conflitos históricos, a
-- requeue aceita exatamente o estado legado available + worker_conflict.
...
begin
  ...
  where profile.organization_id = selected.organization_id
    ...
    and (
      ...
      or (profile.kind = 'canonical' and profile.status = 'available')
    )
  for update;
  if not found then return false; end if;
  ...
  set status = 'callback_received', worker_status = 'pending', worker_id = null,
      ...
      synced_at = null, synced_count = 0, worker_error_code = null,
      worker_error_stage = null, last_error_message = null,
