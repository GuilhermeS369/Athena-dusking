-- O modo diário é uma rotina de calendário: o primeiro horário deve ser o
-- próximo p_daily_time a partir da confirmação, mesmo que existam lotes de
-- outros formatos ativos no perfil. As reservas continuam sendo registradas
-- para o próprio plano diário, mas não deslocam o seu primeiro dia.
...
begin
  ...
  first_daily := case
    when ((date_trunc('day', resolved_now at time zone 'America/Sao_Paulo') + p_daily_time) at time zone 'America/Sao_Paulo') > resolved_now
      then ((date_trunc('day', resolved_now at time zone 'America/Sao_Paulo') + p_daily_time) at time zone 'America/Sao_Paulo')
    else ((date_trunc('day', resolved_now at time zone 'America/Sao_Paulo') + p_daily_time + interval '1 day') at time zone 'America/Sao_Paulo')
  end;

  return jsonb_build_object(
    'profileCount', online_profile_count::text,
    'slotsPerProfile', p_repeat_days::text,
    'expectedPublications', (online_profile_count * p_repeat_days)::text,
    'firstExecuteAt', first_daily,
    'lastExecuteAt', first_daily + ((p_repeat_days - 1)::text || ' days')::interval,
...
begin
  ...
  created := public.create_bulk_rotation_plan(p_organization_id, p_request_key, p_name, p_profile_ids, p_origin_type,
    p_origin_group_id, p_format, 1440, p_repeat_days, p_caption, p_order_mode, p_rotation_seed, p_algorithm_version, p_chunk_size, resolved_now);
  ...
  for profile_row in select * from public.bulk_publication_plan_profiles where plan_id = v_plan_id for update loop
    ...
    set schedule_base_at = first_daily - interval '1 day', first_execute_at = first_daily,
      last_execute_at = first_daily + ((p_repeat_days - 1)::text || ' days')::interval, total_slot_count = p_repeat_days
    where id = profile_row.id;
    update public.bulk_publication_profile_horizons
    set reserved_from = first_daily - interval '1 day', first_execute_at = first_daily,
      reserved_through = first_daily + ((p_repeat_days - 1)::text || ' days')::interval, slot_count = p_repeat_days
    where plan_profile_id = profile_row.id and status = 'active';
  end loop;
