-- Reparo pontual do lote diário criado antes da correção da regra de calendário.
-- É transacional e limita explicitamente o escopo ao lote informado, para não
-- alterar outros planos diários ou itens já publicados.
...
begin
  ...
  if plan_row.id is null
    ...
    or plan_row.format <> 'story'
    or plan_row.schedule_mode <> 'daily_time'
    or plan_row.daily_time <> '07:00:00'::time
    or plan_row.duration_days <> 2
    or plan_row.slots_per_profile <> 2 then
  ...
  from public.bulk_publication_plan_profiles
  where plan_id = plan_row.id;
  if profile_count <> plan_row.profile_count then
    raise exception using errcode = '23514', message = 'A quantidade de perfis do plano não confere.';
  end if;
  if exists (select 1 from public.bulk_publication_plan_profiles where plan_id = plan_row.id and total_slot_count <> 2) then
    raise exception using errcode = '23514', message = 'O plano contém uma quantidade inesperada de slots.';
  end if;

  last_execute_at := p_first_execute_at + interval '1 day';
  update public.bulk_publication_plan_profiles
  set schedule_base_at = p_first_execute_at - interval '1 day', first_execute_at = p_first_execute_at, last_execute_at = last_execute_at
  where plan_id = plan_row.id;
  update public.bulk_publication_profile_horizons
  set reserved_from = p_first_execute_at - interval '1 day', first_execute_at = p_first_execute_at, reserved_through = last_execute_at
  where plan_id = plan_row.id and status = 'active';

  select count(*)::bigint into active_item_count
  from public.publication_items item
  where item.batch_id = plan_row.batch_id
  ...
  return jsonb_build_object('planId', plan_row.id, 'profileCount', profile_count::text,
    'updatedItemCount', updated_item_count::text, 'firstExecuteAt', p_first_execute_at, 'lastExecuteAt', last_execute_at);
end;
$$;
