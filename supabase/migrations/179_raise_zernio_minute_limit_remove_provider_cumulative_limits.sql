-- A Zernio pode despachar até 200 publicações por minuto por organização.
-- Limites acumulados por organização/provedor não acompanham esse throughput e
-- são removidos; permanecem os guardrails por perfil e a reserva concorrente.

alter table public.publication_rate_limit_settings
  drop column if exists max_provider_publications_per_hour,
  drop column if exists max_provider_publications_per_day;

update public.publication_rate_limit_settings
set max_provider_publications_per_minute = 200,
    updated_at = timezone('utc', now()),
    metadata = metadata || jsonb_build_object(
      'source', '179_raise_zernio_minute_limit_remove_provider_cumulative_limits',
      'scope', 'global_zernio_default'
    )
...
select null, 'zernio', true, 200, 100, 45, 300,
  jsonb_build_object(
    'source', '179_raise_zernio_minute_limit_remove_provider_cumulative_limits',
    'scope', 'global_zernio_default'
  )
