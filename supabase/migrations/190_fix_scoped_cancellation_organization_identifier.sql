-- Corrige a ambiguidade PL/pgSQL entre a variável local e a coluna organization_id.`r`n-- A rotina anterior abortava antes da mutação e mantinha a operação durável em andamento.`r`n`r`n-- Cancelamento atÃ´mico da fila operacional por conta, lote ou grupo.
--
-- A rotina nunca confirma um cancelamento enquanto houver uma publicaÃ§Ã£o jÃ¡
-- sob lease de um worker. Nessa situaÃ§Ã£o ela nÃ£o altera nenhum item e devolve
-- `state = blocked`; isso evita afirmar que uma chamada jÃ¡ em curso ao provedor
-- externo foi interrompida. Itens ainda aguardando sÃ£o travados, cancelados e
-- verificados na mesma transaÃ§Ã£o.
...
begin
  if p_scope not in ('account', 'batch', 'group') then
    raise exception using errcode = '22023', message = 'Escopo de cancelamento invÃ¡lido.';
  end if;

  if auth.uid() is null then
    raise exception using errcode = '42501', message = 'AutenticaÃ§Ã£o necessÃ¡ria.';
  end if;
  ...
  if p_scope = 'batch' then
    ...
    if target_organization_id is null then
      raise exception using errcode = 'P0002', message = 'Lote de publicaÃ§Ã£o nÃ£o encontrado.';
    end if;
    ...
    if target_organization_id is null then
      raise exception using errcode = 'P0002', message = 'Perfil nÃ£o encontrado.';
    end if;
  else
    ...
    if target_organization_id is null then
      raise exception using errcode = 'P0002', message = 'Grupo nÃ£o encontrado.';
    end if;
  ...
  if not public.has_organization_role(target_organization_id, array['admin', 'operator']::public.organization_role[]) then
    raise exception using errcode = '42501', message = 'AÃ§Ã£o nÃ£o permitida.';
  end if;
  ...
  -- MantÃ©m a ordem de lock do worker de geraÃ§Ã£o: chunks antes de job. Caso um
  -- chunk jÃ¡ esteja em execuÃ§Ã£o, esta transaÃ§Ã£o espera seu commit, cancela os
  -- itens que ele acabou de materializar e sÃ³ entÃ£o altera os chunks restantes.
  perform 1
  ...
  -- O payload Ã© a fonte de verdade antes da materializaÃ§Ã£o dos chunks.
  -- Um worker que jÃ¡ recebeu lease pode estar em uma chamada externa. Em vez
  -- de apagÃ¡-lo da fila e dar uma falsa garantia, abortamos integralmente.
  for item_row in
    select item_source.*
  ...
  if coalesce(array_length(blocked_item_ids, 1), 0) > 0 then
    return jsonb_build_object(
      ...
      'message', 'HÃ¡ publicaÃ§Ã£o(Ãµes) jÃ¡ em processamento. Nenhum item foi cancelado; aguarde a finalizaÃ§Ã£o e tente novamente.'
    );
  end if;

  -- Para lote, o job inteiro pertence ao lote e precisa ser encerrado. Para
  -- conta/grupo, conservamos o job para os demais perfis e gravamos a exclusÃ£o
  -- que tambÃ©m Ã© respeitada pelo materializador e pelos workers de chunk.
  for job_row in
  ...
  loop
    ...
    if p_scope = 'batch' or not job_has_remaining_profiles then
      ...
      -- Esses chunks jÃ¡ estÃ£o travados acima. Remover o perfil do payload
