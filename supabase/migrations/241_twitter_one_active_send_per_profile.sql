-- Um perfil X nunca pode ter dois envios simultâneos. Itens aceitos/processando
-- e resultados incertos continuam bloqueando novos claims até reconciliação.
do $$
begin
  if exists (
    select profile_id
    ...
    raise exception using errcode='23505',message='Há mais de um envio X ativo para o mesmo perfil.';
  end if;
end $$;

drop index if exists public.twitter_one_claim_per_profile_idx;
create unique index twitter_one_active_send_per_profile_idx
on public.twitter_publication_items(profile_id)
where status in ('claimed','processing','outcome_unknown');
  ...
  with candidates as (
    ...
    where queue_item.status in ('ready','retry')
      ...
      and not exists (
        ...
        where active_item.profile_id=queue_item.profile_id
          and active_item.status in ('claimed','processing','outcome_unknown')
      )
      and queue_item.id=(
        ...
        order by case when first_item.status='retry' then 0 else 1 end,
          coalesce(first_item.next_attempt_at,first_item.execute_at),first_item.id limit 1
      )
