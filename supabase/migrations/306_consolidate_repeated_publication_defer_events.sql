-- O estado autoritativo e o contador continuam atualizados em todo polling,
-- mas a trilha operacional registra apenas o primeiro e marcos a cada cinco
-- polls. Deferimentos de criação/recovery continuam sempre registrados.
...
begin
  ...
  if not p_is_poll
    or item_row.container_poll_count = 0
    or updated_row.container_poll_count % 5 = 0 then
    perform public.log_publication_item_event(
      ...
      jsonb_build_object(
        ...
        'consolidated_poll_events', p_is_poll
      )
    );
  end if;
