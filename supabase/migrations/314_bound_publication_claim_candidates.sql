-- Mantém o claim crítico restrito à fila quente. O contrato público não muda.
-- Criações aceitas e itens ainda não enviados usam índices distintos para que
-- o banco não filtre toda a agenda futura a cada ciclo de quatro itens.

create index if not exists publication_items_provider_accepted_claim_idx
  on public.publication_items (execute_at, next_attempt_at, lease_until, organization_id, profile_id, id)
  include (created_at, batch_id, status)
  where archived_at is null
    and creation_id is not null
    and status in ('ready', 'waiting', 'preparing', 'failed');

create index if not exists publication_items_unstarted_current_claim_idx
  on public.publication_items (execute_at, next_attempt_at, lease_until, organization_id, profile_id, id)
  include (created_at, batch_id, pipeline_version, preparation_status, zernio_recovery_count, status)
  where archived_at is null
...
begin
  ...
  with eligible_source as (
    ...
    where item.archived_at is null
      ...
      and (item.status <> 'failed' or (item.attempt_count < 5 and item.next_attempt_at is not null))
      and (item.execute_at is null or item.execute_at <= timezone('utc', now()))
      and (item.execute_at is null or item.execute_at >= timezone('utc', now()) - interval '60 seconds')
      and (item.next_attempt_at is null or item.next_attempt_at <= timezone('utc', now()))
      and (item.lease_until is null or item.lease_until <= timezone('utc', now()))
    ...
    select source.*,
      row_number() over (
        ...
        order by source.priority_band, coalesce(source.execute_at, source.created_at), source.id
      ) as org_position,
      row_number() over (
    ...
    set status = 'preparing',
        ...
        attempt_count = item.attempt_count
          + case when item.creation_id is null or item.status = 'failed' then 1 else 0 end
    from locked
