-- O filtro por data de adicao passa a aceitar intervalo.
--
-- p_created_on continua aceito de proposito. O app em producao neste instante
-- chama a funcao com ele; se a assinatura perdesse o parametro no push, /perfis
-- devolveria erro ate o deploy da Vercel terminar -- uns dois minutos de painel
-- quebrado. O parametro sai na migration seguinte, depois que a versao nova
-- estiver no ar. Enquanto existe, vale como intervalo de um dia so.
--
-- O recorte continua sendo intervalo semiaberto sobre a coluna crua, convertido
-- para America/Sao_Paulo nas duas pontas: >= inicio e < fim + 1 dia. Assim o
-- indice de (created_at, id) segue valendo e "adicionadas em 27" continua
-- significando o dia 27 de quem opera, nao o dia 27 em UTC.
--
-- As tres funcoes mudam juntas: pagina, ids e resumo precisam do mesmo recorte,
-- senao "selecionar todos deste filtro" apaga um conjunto diferente do exibido.
...
as $$
  with normalized as (
    ...
    where profile.organization_id = p_organization_id
      ...
      and (
        coalesce(p_created_from, p_created_on) is null
        or profile.created_at >= (coalesce(p_created_from, p_created_on)::timestamp at time zone 'America/Sao_Paulo')
      )
      and (
        coalesce(p_created_to, p_created_on) is null
        or profile.created_at < ((coalesce(p_created_to, p_created_on) + 1)::timestamp at time zone 'America/Sao_Paulo')
      )
...
revoke all on function public.list_instagram_profiles_catalog_page(uuid, integer, timestamptz, uuid, text, uuid, text, text, text, text, bigint, date, date, date) from public, anon;
grant execute on function public.list_instagram_profiles_catalog_page(uuid, integer, timestamptz, uuid, text, uuid, text, text, text, text, bigint, date, date, date) to authenticated, service_role;
...
revoke all on function public.list_instagram_profiles_catalog_ids(uuid, integer, text, uuid, text, text, text, date, date, date) from public, anon;
grant execute on function public.list_instagram_profiles_catalog_ids(uuid, integer, text, uuid, text, text, text, date, date, date) to authenticated, service_role;
...
revoke all on function public.get_instagram_profiles_catalog_summary(uuid, text, uuid, text, text, text, date, date, date) from public, anon;
grant execute on function public.get_instagram_profiles_catalog_summary(uuid, text, uuid, text, text, text, date, date, date) to authenticated, service_role;
