-- Remove p_created_on das funcoes do catalogo.
--
-- Ele existia so para a janela do deploy da 345: a versao no ar naquele momento
-- chamava por ele, e derruba-lo junto teria deixado /perfis quebrado ate a
-- Vercel terminar de publicar. O deploy dpl_ucfRK7JSUdvxkNYcjBKFFC1ANVrA ja
-- esta no alias oficial e chama por p_created_from/p_created_to, entao o
-- parametro nao tem mais chamador.
--
-- Manter um parametro morto numa assinatura que ja tem doze custa mais do que
-- esta migration: o proximo a ler teria de descobrir sozinho qual dos tres
-- caminhos de data vale.
--
-- supabase/tests/344_instagram_profiles_catalog_created_on.test.sql sai junto,
-- no mesmo commit. Ele exercitava p_created_on por posicao, e o cenario dele --
-- fronteira de fuso entre 26 e 28/08 -- e um subconjunto do teste da 345.
...
drop function if exists public.list_instagram_profiles_catalog_page(uuid, integer, timestamptz, uuid, text, uuid, text, text, text, text, bigint, date, date, date);
drop function if exists public.list_instagram_profiles_catalog_page(uuid, integer, timestamptz, uuid, text, uuid, text, text, text, text, bigint, date, date);
  ...
  with normalized as (
    ...
    where profile.organization_id = p_organization_id
      ...
      and (
        ...
        or profile.created_at >= (p_created_from::timestamp at time zone 'America/Sao_Paulo')
      )
      and (
        p_created_to is null
        or profile.created_at < ((p_created_to + 1)::timestamp at time zone 'America/Sao_Paulo')
      )
...
revoke all on function public.list_instagram_profiles_catalog_page(uuid, integer, timestamptz, uuid, text, uuid, text, text, text, text, bigint, date, date) from public, anon;
grant execute on function public.list_instagram_profiles_catalog_page(uuid, integer, timestamptz, uuid, text, uuid, text, text, text, text, bigint, date, date) to authenticated, service_role;
...
drop function if exists public.list_instagram_profiles_catalog_ids(uuid, integer, text, uuid, text, text, text, date, date, date);
drop function if exists public.list_instagram_profiles_catalog_ids(uuid, integer, text, uuid, text, text, text, date, date);
...
revoke all on function public.list_instagram_profiles_catalog_ids(uuid, integer, text, uuid, text, text, text, date, date) from public, anon;
grant execute on function public.list_instagram_profiles_catalog_ids(uuid, integer, text, uuid, text, text, text, date, date) to authenticated, service_role;
...
drop function if exists public.get_instagram_profiles_catalog_summary(uuid, text, uuid, text, text, text, date, date, date);
drop function if exists public.get_instagram_profiles_catalog_summary(uuid, text, uuid, text, text, text, date, date);
...
revoke all on function public.get_instagram_profiles_catalog_summary(uuid, text, uuid, text, text, text, date, date) from public, anon;
grant execute on function public.get_instagram_profiles_catalog_summary(uuid, text, uuid, text, text, text, date, date) to authenticated, service_role;
