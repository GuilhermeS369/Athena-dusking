...
select extensions.throws_ok(format($q$select public.twitter_confirm_bulk_program('22800000-0000-4000-8000-000000000001','12800000-0000-4000-8000-000000000001','confirm-228-stale',%L,1,%L::jsonb,%L::jsonb,'[]','[]',%L::jsonb,'[]')$q$,repeat('b',64),jsonb_build_array(jsonb_build_object('identityId',(select identity_id from twitter_228_context),'walletVersion',1,'availableMicros',12000000))::text,jsonb_build_object('scheduleKind','interval','startsAt','2027-01-01T12:00:00Z','endsAt','2027-01-01T12:00:00Z','intervalMinutes',1,'totalRequested',1,'unfundedCount',0)::text,jsonb_build_array(jsonb_build_object('profile_id',(select profile_id from twitter_228_context),'connection_epoch_id',(select epoch_id from twitter_228_context),'connection_id',(select connection_id from twitter_228_context),'identity_id',(select identity_id from twitter_228_context),'slot_index',0,'execute_at','2027-01-01T12:00:00Z','content','x','weighted_characters',1,'media_set_client_key',null,'category','post_dm_create','amount_micros',15000))::text),'40001','Saldo ou reservas mudaram; revise novamente.','snapshot antigo recebe conflito');
...
select jsonb_build_object(
  'finish',(select jsonb_agg(result) from extensions.finish() result),
  'programs',(select count(*) from public.twitter_programs),
  'items',(select count(*) from public.twitter_publication_items),
  'shortfall',(select unfunded_count from public.twitter_program_shortfalls),
  'reserved',(select reserved_micros from public.twitter_wallets where identity_id=(select identity_id from twitter_228_context)),
  'reservations',(select count(*) from public.twitter_wallet_reservations),
  'posted',(select posted_balance_micros from public.twitter_wallets where identity_id=(select identity_id from twitter_228_context)),
  'anonExecute',has_function_privilege('anon','public.twitter_confirm_bulk_program(uuid,uuid,text,text,integer,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb)','EXECUTE'),
  'authenticatedExecute',has_function_privilege('authenticated','public.twitter_confirm_bulk_program(uuid,uuid,text,text,integer,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb)','EXECUTE'),
  'serviceExecute',has_function_privilege('service_role','public.twitter_confirm_bulk_program(uuid,uuid,text,text,integer,jsonb,jsonb,jsonb,jsonb,jsonb,jsonb)','EXECUTE'),
  'instagramForeignKeys',(select count(*) from pg_constraint c join pg_class t on t.oid=c.conrelid join pg_class f on f.oid=c.confrelid where t.relname like 'twitter_%' and f.relname in ('instagram_profiles','publication_items'))
) as diagnostics; rollback;
